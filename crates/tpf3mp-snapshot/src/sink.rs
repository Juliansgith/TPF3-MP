...
use crate::{ChunkEntry, ChunkError, ChunkId, ChunkStore, Manifest, ManifestId, StoreError, codec};
...
/// Receives the chunks of one snapshot, in any order, from any number of
/// peers, and assembles the file once all are present.
///
/// Every chunk is checked before it is stored: it must belong to the
/// manifest, fit the zstd bound for its length, decompress to exactly that
/// length, and hash to its id. Chunks the store already holds, from earlier
/// snapshots or other transfers, are never fetched again.
///
...
/// collection keeps the chunks of unfinished transfers.
///
/// Its methods do blocking file I/O; async callers run them on a blocking
/// thread.
...
impl ChunkSink {
    ...
    pub fn resume(store: &ChunkStore, id: &ManifestId) -> Result<Self, SinkError> {
        let manifest = store.load_pending(id)?;
        Self::start(store, manifest)
    }

    fn start(store: &ChunkStore, manifest: Manifest) -> Result<Self, SinkError> {
    ...
    pub fn put(&mut self, id: &ChunkId, frame: &[u8]) -> Result<Progress, SinkError> {
        ...
        codec::decode(&mut self.decompressor, id, raw_len, frame)
    ...
    /// Assembles and verifies the file, publishes it at `dest` atomically,
    /// and forgets the transfer.
    ///
    /// If a stored chunk turns out to be damaged, it is removed and this fails;
    /// [`missing`](Self::missing) then lists it, so fetching it again and
    /// retrying completes the transfer.
    pub fn finish(&mut self, dest: &Path) -> Result<(), SinkError> {
        ...
        if let Err(error) = self.store.assemble(&self.manifest, dest) {
            self.refresh()?;
            return Err(error.into());
        }
        self.finished = true;
        self.store.remove_pending(&self.manifest.id())?;
        Ok(())
    }
